 ...
 */
 function doJSONRequest(type, url, headers, data, callback){
  ...
  doRequestChecks(type, true, data);

  //create an ajax request
  var r = new XMLHttpRequest();

  //open a connection to the server using type method on the url API
  r.open(type, url, true);

  //set the headers
  doRequestSetHeaders(r, type, headers);
...
function doRequestSetHeaders(r, type, headers){

  //set the default JSON header according to the type parameter
...
function doRequestChecks(type, isAsynchronous, data){

  //verify the request type
  if(type!="GET" && type!="POST" && type!="PUT" && type!="DELETE") {
    throw new Error('Illegal type: ' + type + ". It should be one of: GET, POST, PUT, DELETE.");
  }
